...
@section('title')
    {{ __('Edit Blog Post') }} - {{ config('app.name') }}
@endsection

@section('admin-content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <div x-data="{ pageName: '{{ __('Edit Blog Post') }}' }">
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">{{ __('Edit Blog Post') }}</h2>
                <nav>
                    ...
                    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" readonly value="{{ old('slug', $blog->slug) }}"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="excerpt" id="excerpt" rows="3"
                                          class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white/90 dark:placeholder:text-white/30">{{ old('excerpt', $blog->excerpt) }}</textarea>
                            </div>

                            <div class="sm:col-span-2">
                                <label for="content" class="block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Content') }}</label>
                                <textarea name="content" id="content" rows="6"
                                          class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white/90 dark:placeholder:text-white/30">{{ old('content', $blog->content) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                @if($blog->thumbnail)
                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Current Thumbnail:') }}</p>
                                    <img src="{{ asset('storage/'.$blog->thumbnail) }}" alt="Thumbnail" class="mt-2 rounded w-40 h-auto border">
                                @endif
                            </div>
