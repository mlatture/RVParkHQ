...
@section('title')
    {{ __('Park Requests') }} | {{ config('app.name') }}
@endsection

@section('admin-content')

    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <div x-data="{ pageName: {{ __('Park Requests') }} }">
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                ...
                <nav>
                    <ol class="flex items-center gap-1.5">
                        ...
                        <li class="text-sm text-gray-800 dark:text-white/90">{{ __('Park Requests') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between items-center">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white/90">{{ __('Park Requests') }}</h3>
                ...
                <div class="space-y-3 border-t border-gray-100 dark:border-gray-800 overflow-x-auto">
                    ...
                    <table id="dataTable" class="w-full dark:text-gray-400">
                        ...
                        <tbody>
                        @forelse ($ParkEditRequests as $index => $ParkEditRequest)
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                <td class="px-5 py-4 sm:px-6">{{ $index + 1 }}</td>
                                <td class="px-5 py-4 sm:px-6">{{ $ParkEditRequest->park->name }}</td>
                                <td class="px-5 py-4 sm:px-6">{{ $ParkEditRequest->owner->email }}</td>
                                ...
                                <td class="flex px-5 py-4 sm:px-6 text-center gap-1">
                                    <a data-tooltip-target="tooltip-edit-user-{{ $ParkEditRequest->id }}" class="btn-default !p-3"
                                       href="{{ route('admin.park-request.edit', $ParkEditRequest->id) }}">
                                    ...
                                    <div id="delete-modal-{{ $ParkEditRequest->id }}" tabindex="-1"
                                        ...
                                        <div
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{ __('Are you sure you want to delete this Park Request?') }}</h3>
                                                <form id="delete-form-{{ $ParkEditRequest->id }}"
                                                      action="{{ route('admin.park-request.destroy', $ParkEditRequest->id) }}"
                                                      method="POST">
                                                    @method('DELETE')
                                                    @csrf
