        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                <div class="px-5 py-2.5 sm:px-6 sm:py-5">
                    <h3 class="text-base font-medium text-gray-800 dark:text-white">{{ __('Edit Park Request') }} -
                        {{ $ParkEditRequest->name }}</h3>
                </div>
                <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                    @include('backend.layouts.partials.messages')
                    <form action="{{ route('admin.park-request.update', $ParkEditRequest->id) }}" method="POST" class="space-y-6">
                        @method('PUT')
                        @csrf
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                <label for="name"
                                       class="block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Park Name') }}</label>
                                <input type="text" name="name" id="name" required value="{{ $ParkEditRequest->park->name }}"
                                       placeholder="{{ __('Enter Park Name') }}"
                            ...
                            <div>
                                <label for="email"
                                       class="block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('Owner Email') }}</label>
                                <input type="email" name="email" id="email" required value="{{ $ParkEditRequest->owner->email }}"
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    ...
                                    @foreach (['pending', 'approved', 'rejected'] as $status)
                                        <option value="{{ $status }}" {{ $ParkEditRequest->status === $status ? 'selected' : '' }}>
                                            {{ ucfirst($status) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {!! ld_apply_filters('after_username_field', '', $ParkEditRequest) !!}
                        </div>
